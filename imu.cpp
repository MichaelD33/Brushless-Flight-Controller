...
void imuCombine(){
     ...
     angle.y = GYRO_PART * (angle.y + (gyroRates.x * SAMPLETIME_S)) + (1-GYRO_PART) * pitch; //complementary filter
     angle.x = GYRO_PART * (angle.x + (gyroRates.y * SAMPLETIME_S)) + (1-GYRO_PART) * roll;
     //angle.z = angle.z + (gyroRates.z * SAMPLETIME_S);
     angle.z = (gyroRates.z * SAMPLETIME_S);
